...
import {
  createRetalhoTableRow,
  createRetalhoCard,
  createReservedItemsTable,
  createHistoryLog,
} from "./components.js";
...
export function populateSelect(
  selectElement,
  items,
  { defaultOption, textKey, valueKey, addOptions = [] },
) {
...
export function updatePagination(currentPage, totalItems, itemsPerPage) {
  ...
  if (totalPages > 1) {
    get(SELECTORS.pageInfo).textContent =
...
export function toggleClearButtonVisibility() {
  ...
  const hasFilter =
    ...
    form.elements.espessura.value ||
    form.elements.largura.value ||
