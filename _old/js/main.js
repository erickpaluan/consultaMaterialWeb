...
export function initializeApp() {
  ...
  get(SELECTORS.smartSearchInput).addEventListener(
    "input",
    handlers.debouncedSmartSearch,
  );
  get(SELECTORS.filterForm).addEventListener("submit", (e) =>
    e.preventDefault(),
  );
  get(SELECTORS.materialSelect).addEventListener(
    "change",
    handlers.handleFilterFormChange,
  );
  get(SELECTORS.tipoSelect).addEventListener(
    "change",
    handlers.handleFilterFormChange,
  );
  get(SELECTORS.filterForm)
    .querySelectorAll("input")
    .forEach((input) => {
      input.addEventListener("input", handlers.handleFilterFormChange);
    });
  get(SELECTORS.clearBtn).addEventListener(
    "click",
    handlers.handleClearFilters,
  );
  get(SELECTORS.prevPageBtn).addEventListener("click", () =>
    handlers.handlePageChange(-1),
  );
  get(SELECTORS.nextPageBtn).addEventListener("click", () =>
    handlers.handlePageChange(1),
  );
  get(SELECTORS.resultsTable).addEventListener("click", handlers.handleSort);
  get(SELECTORS.openRegisterModalBtn).addEventListener(
    "click",
    handlers.handleOpenRegisterModal,
  );
  get(SELECTORS.showReservedBtn).addEventListener("click", () =>
  ...
  get(SELECTORS.closeRegisterModalBtn).addEventListener("click", () =>
    closeModal(get(SELECTORS.registerModal)),
  );
  get(SELECTORS.regSubmitBtn).addEventListener(
    "click",
    handlers.handleRegisterSubmit,
  );
  form.elements["reg-material"].addEventListener(
    "change",
    handlers.handleMaterialCadastroChange,
  );
  form.elements["reg-tipo"].addEventListener(
    "change",
    handlers.handleTipoCadastroChange,
  );
  get(SELECTORS.regSalvarNovoMaterialBtn).addEventListener(
    "click",
    handlers.handleSalvarNovoMaterial,
  );
  get(SELECTORS.regSalvarNovoTipoBtn).addEventListener(
    "click",
    handlers.handleSalvarNovoTipo,
  );
  get(SELECTORS.regCancelarNovoMaterialBtn).addEventListener("click", () =>
    get(SELECTORS.regNovoMaterialContainer).classList.add("hidden"),
  );
  get(SELECTORS.regCancelarNovoTipoBtn).addEventListener("click", () =>
    get(SELECTORS.regNovoTipoContainer).classList.add("hidden"),
  );
  get(SELECTORS.regClearBtn).addEventListener(
    "click",
    handlers.handleClearRegisterForm,
  );

  get(SELECTORS.closeReserveModalBtn).addEventListener("click", () =>
    closeModal(get(SELECTORS.reserveModal)),
  );
  get(SELECTORS.reserveConfirmBtn).addEventListener(
    "click",
  ...
  get(SELECTORS.closeModalBtn).addEventListener("click", () =>
    closeModal(get(SELECTORS.reservedModal)),
  );
  get(SELECTORS.osSearchInput).addEventListener(
    "input",
    handlers.handleSearchReserved,
  );
  get(SELECTORS.createPdfBtn).addEventListener(
    "click",
    handlers.handleGeneratePdf,
  );

  get(SELECTORS.closeHistoryModalBtn).addEventListener("click", () =>
